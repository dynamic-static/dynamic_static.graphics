...
namespace Vulkan {
    ...
    ShaderModule::Compiler::Compiler(
        VkShaderStageFlagBits stage,
        const std::string& source
    )
        : ShaderModule::Compiler(stage, 0, source)
    {
    }
    ...
    )
    {
        ...
        glslang::GlslangToSpv(*program.getIntermediate(eshStage), mSpirv);
    }

    VkShaderStageFlagBits ShaderModule::Compiler::get_stage() const
    {
        return mStage;
    }

    dst::Span<const uint32_t> ShaderModule::Compiler::get_spirv() const
    {
        return mSpirv;
    }
